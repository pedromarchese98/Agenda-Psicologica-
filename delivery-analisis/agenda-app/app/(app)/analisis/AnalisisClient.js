...
export default function AnalisisClient({ appointments, activeCount, events }) {
  ...
  const stats = useMemo(() => {
    ...
    filtered.forEach((a) => {
      ...
      if (!byPatient[name]) byPatient[name] = { name, total: 0, att: 0, cancDay: 0, cancAdv: 0, cancFree: 0, paid: 0, debt: 0, status: a.patients?.status || '', lastVisit: a.date };
      byPatient[name].total++;
      if (a.date > byPatient[name].lastVisit) byPatient[name].lastVisit = a.date;

      const weekday = (new Date(a.date + 'T00:00:00').getDay() + 6) % 7;
      totalByWeekday[weekday]++;

      if (a.attendance === 'yes') att++, byPatient[name].att++;
      if (a.attendance === 'no') {
      ...
      if (a.attendance === 'no-free') { cancFree++; byPatient[name].cancFree++; byWeekday[weekday]++; }
      if (a.payment === 'paid') {
  ...
  const DEFAULT_ON = ['name', 'total', 'att', 'cancDay', 'cancAdv', 'paid', 'debt'];
